...
//		Date date = sdf.parse(strDate);
    ...
    void selectQuotationByUserTest() throws ParseException {
        ...
        // Entity를 DTO로 변환 후 비교
        ReqQuotationDTO expectedDto = ReqQuotationDTO.builder()
                .quotationReqNo(mockEntity.getQuotationReqNo())
                .custId(mockEntity.getCustId())
                .pickupAddr(mockEntity.getPickupAddr())
                .deliveryAddr(mockEntity.getDeliveryAddr())
                .moveDt(mockEntity.getMoveDt())
                .buildingType(mockEntity.getBuildingType())
                .roomStructure(mockEntity.getRoomStructure())
                .houseSize(mockEntity.getHouseSize())
                .hasElevator(true)
                .boxCount(mockEntity.getBoxCount())
                .quotationAmount(mockEntity.getQuotationAmount())
                .status(mockEntity.getStatus())
                .version(mockEntity.getVersion())
                .build();
...
//
//        // when: Mock the service method behavior
...
//
//        // then: Assert that the returned list matches the expected one
//        assertThat(list).containsExactlyInAnyOrderElementsOf(mockList);
//    }
